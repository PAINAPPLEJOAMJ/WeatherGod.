...
public class Port_System : FSM_Base
{
    ...
    void Start()
    {
        points = GameObject.Find("Port_PointGroup").GetComponentsInChildren<Transform>();
    ...
    IEnumerator createPerson () 
    {
        while (true) 
        {
            ...
            if (pCnt < maxP) 
            {
                ...
                var P_Person = Instantiate(person[pIdx],
                ...
                P_Person.transform.parent = gameObject.transform;
            } 
            else if (pCnt == maxP) 
            {
                break;
