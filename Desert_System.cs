...
public class Desert_System : FSM_Base
{
    ...
    void Start()
    {
        points = GameObject.Find("Desert_PointGroup").GetComponentsInChildren<Transform>();
    ...
    IEnumerator createPerson()
    {
        while (true)
        {
            ...
            if (pCnt < maxP)
            {
                ...
                var D_Person = Instantiate (person[pIdx],
                ...
                D_Person.transform.parent = gameObject.transform;
            }
            else if (pCnt == maxP)
            {
                break;
